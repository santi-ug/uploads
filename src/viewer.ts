...
<section id="thread-popover" role="dialog" aria-label="Comment thread" hidden><button id="thread-close" aria-label="Close thread">×</button><div id="thread-content"></div><div id="thread-actions"></div></section>
<nav id="tools" class="island" aria-label="Annotation tools">${tool('read', 'Read and scroll')}${tool('select', 'Select element')}${tool('rect', 'Mark region')}${tool('ellipse', 'Circle')}${tool('stroke', 'Draw')}${tool('erase', 'Erase mark')}${tool('undo', 'Undo mark')}${tool('redo', 'Redo mark')}<button type="button" id="save" aria-label="Save and send feedback to your agent" title="Save and send feedback to your agent"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20 6 9 17l-5-5"/></svg></button></nav><div id="save-notice" role="status" aria-live="polite" hidden></div><p id="instruction" role="status">Select text to comment, or choose a markup tool.</p><button id="draft" hidden>Resume draft</button><div id="snapshot-note" hidden>Saved HTML at its original viewport. Live interaction state is not captured.</div></body></html>`, { headers: {
    'content-type': 'text/html; charset=utf-8',
